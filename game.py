    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        while num_player<NUMBER_OF_PLAYERS:
            ...
            try:
                ...
                player_process = multiprocessing.Process(target=player_handler, args = (player_socket, address, num_player, NUMBER_OF_PLAYERS,))
    ...
    with player_socket:
            ...
            if num_player==shared_memory._getvalue()[f"tour"]%NUMBER_OF_PLAYERS:
                ...
                if "mes" not in play_action:
                    ...
                    #test fin de jeu perdant
                    if len(suite)>=5 and suite not in suites_gagnantes:
                        shared_memory.update({'game_over':True})
